	...
	<div class="w-full">
		...
		<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
			...
			<div class="flex flex-col gap-1.5">
				...
				<UInput v-model="filter.query" :placeholder="t('components.filter.searchOutlet')" :icon="ICONS.SEARCH_ROUNDED" @input="debouncedSearch" />
			</div>
...
const { filter } = storeToRefs(outletStore);

const is_loading = computed(() => outletStore.loading);
