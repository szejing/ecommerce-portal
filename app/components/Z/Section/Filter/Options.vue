	...
	<div class="w-full">
		...
		<div class="grid grid-cols-3 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
			...
			<div class="flex flex-col col-span-3 sm:col-span-2 gap-1.5">
				...
				<UInput v-model="filter.query" :placeholder="t('components.filter.searchOptions')" :icon="ICONS.SEARCH_ROUNDED" @input="debouncedSearch" />
			</div>
...
const { filter } = storeToRefs(productOptionStore);

const is_loading = computed(() => productOptionStore.loading);
