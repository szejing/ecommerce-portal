	...
	<div class="w-full">
		<div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
			<div class="flex flex-col gap-1.5 col-span-2">
				...
				<UInput v-model="filter.query" :placeholder="t('components.filter.searchCouriers')" :icon="ICONS.SEARCH_ROUNDED" @input="debouncedSearch" />
			</div>
...
const { filter } = storeToRefs(courierStore);

const is_loading = computed(() => courierStore.loading);
