	...
	<div class="w-full">
		<div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
			<div class="flex flex-col gap-1.5 col-span-2">
				...
				<UInput v-model="filter.query" :placeholder="t('components.filter.searchShippingZones')" :icon="ICONS.SEARCH_ROUNDED" @input="debouncedSearch" />
			</div>
...
import { SHIPPING_ZONE_STATUS_FILTER_ALL, getShippingZoneStatusFilterItems } from '~/utils/options';
import { capitalizeFirstLetter } from '~/utils/utils';
import { ICONS } from '~/utils/icons';

const { t } = useI18n();
const zoneStore = useShippingZoneStore();
const { filter } = storeToRefs(zoneStore);

const is_loading = computed(() => zoneStore.loading);
const searchTimeout = ref<ReturnType<typeof setTimeout> | null>(null);

const statusItems = computed(() => getShippingZoneStatusFilterItems(t));

const statusSelectValue = computed(() => filter.value.status ?? SHIPPING_ZONE_STATUS_FILTER_ALL);
...
const onStatusChange = async (value: string | undefined) => {
	if (!value || value === SHIPPING_ZONE_STATUS_FILTER_ALL) {
